"""
cortex_memory.legacy — LOAD-PATH ONLY.  Do not delete.  Do not select for a new run.

These four buffers are superseded by the Prefix* pair in `buffers.py`.  They are
kept for exactly one reason, and it is not sentiment:

    Every Track-A and B1 checkpoint carries `memory_slots` / `accum_ccot` /
    `gated_accum` / `ccot_direct` in its config.json, and `cortex_graft.py` is
    the LOAD path for all of them.  If these classes go, every number in the
    historical results table becomes un-re-measurable.

That is not hypothetical.  From 2026-08-02 to 2026-08-04 `CortexMemory.__init__`
*raised* on those flags, and the entire historical table could not be opened --
the write-capacity diagnostic could not load the very checkpoint whose -0.01282
it had been written to explain.  The fix was to move the guard, not the code:
the graft BUILDS these (load/eval path), and `train.py.__post_init__` REFUSES to
start a new run on one.  Building a real buffer cannot produce the silent
no-memory null that the raise was defending against.

Quarantined into this module on 2026-09-14 so that `buffers.py` shows only the
live surface.  Behaviour is unchanged and `buffers.py` re-exports every name, so
existing imports keep working.
  ...
  GatedAccumBuffer -- gated-accumulation LM2 variant (extraction write + LM2
                      gated merge on K fixed slots)

`_extract_summary_vectors` lives here too: it is the shared extraction attention
for AccumCCoT and GatedAccumBuffer, and the Prefix* buffers do not use it (their
write is the host model's own forward pass, which is infidelity #2 in the
`buffers.py` header).
"""
