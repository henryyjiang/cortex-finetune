...
# --- cortex memory graft (flag-gated; default off) -----------------------------
# Imported from the retrofitting-recurrence repo root.  When this modeling file
# is loaded standalone from a bare HF snapshot that lacks the cortex files,
# memory is simply unavailable and the model runs as the unmodified base.
try:
    from cortex_graft import CortexMemory, memory_enabled, build_loop_lora
except Exception:  # pragma: no cover - snapshot without cortex files
    ...
    try:
        ...
        CortexMemory = None
        build_loop_lora = None
        def memory_enabled(config):  # type: ignore
            return False
# -------------------------------------------------------------------------------
...
class CausalLMOutputRecurrentLatents(ModelOutput):
    ...
    m_cross: Optional[torch.Tensor] = None  # cortex: carried cross-segment memory
...
class RavenForCausalLM(RavenPreTrainedModel, GenerationMixin):
    ...
    def __init__(
        ...
        # --- cortex memory graft (flag-gated; default off) ---
        # All settings read from config via getattr defaults (see cortex_graft).
        # self.cortex is None unless config.use_memory is True.
        self.cortex = CortexMemory(config) if (CortexMemory is not None and memory_enabled(config)) else None
        # LoRA-on-loop (rung 1b): additive low-rank hooks on the loop linears,
        # zero-init (B=0) -> exact no-op at step 0.  None unless lora_rank > 0.
        self.cortex_lora = build_loop_lora(self, config) if build_loop_lora is not None else None
    ...
    def forward(
        ...
        # --- cortex memory graft (ignored when self.cortex is None) ---
        ...
        **kwargs,
        ...
        if self.cortex is not None:
            ...
            # would copy noise).  Deferring to first use is the only point
            # where the real base weights are guaranteed loaded.  The
            # already-seeded flag is a persistent buffer, so a resumed or
            # eval-loaded checkpoint does NOT get its trained summary_emb
            # overwritten here.
        ...
        return CausalLMOutputRecurrentLatents(
            ...
            m_cross=new_m_cross if return_m_cross else None,
        )
